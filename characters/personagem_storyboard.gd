...
## Toca UMA animação qualquer numa pose (sem se mover sozinho). Usada pelo
## diretor para momentos especiais: subir pelo braço robótico (ex.: "Jump_Up"),
## lutar com um robô/boss (ex.: "Punch_Side"), ou só ficar parado ("Idle_Side").
## para_direita = para que lado o sprite deve olhar.
func tocar_pose(nome: String, para_direita: bool = true) -> void:
	_dirigido = true
	_parado = true
	_aguardando_inicio = false
	if nome == "Walk_Side" or nome.ends_with("_Side"):
