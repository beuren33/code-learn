...
var _dups: Array = []    # cópias criadas (removidas ao parar)
...
func parar() -> void:
	...
	for item in _itens:
		var no2 = item["no"]
		if is_instance_valid(no2):
			no2.position.x = item["base"]
	_itens.clear()
	...
	if mover_cena_toda:
		...
		for c in pai.get_children():
			...
			if _eh_carro(c):
				continue
			if c.name == nome_bg:
				continue  # o CÉU (fundo) fica parado; só o chão/folhas/estrada deslizam
